    ...
    public List<PaymentTransaction> findAllWithFilters(String search, Long userId, PaymentMethod method, PaymentTransactionStatus status) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            if (search != null && !search.trim().isEmpty()) {
                ...
                Predicate orderSearch = cb.like(cb.lower(cb.function("STR", String.class, transactionRoot.get("order").get("id"))), likePattern);
                predicates.add(cb.or(userSearch, orderSearch));
            }
            if (userId != null) {
            ...
            System.err.println("Error finding all payment transactions with filters: " + e.getMessage());
            e.printStackTrace();
            return null;
        }
    }
