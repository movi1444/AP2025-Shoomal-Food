...
public class AdminCouponHandler extends AbstractHttpHandler {
        ...
        try {
            ...
            if (path.equals("/admin/coupons")) {
                ...
                if (method.equalsIgnoreCase("DELETE")) {
                    ...
                    sendResponse(exchange, HttpURLConnection.HTTP_BAD_METHOD,
                            new ApiResponse(false, "Method Not Allowed for /admin/coupons/{id}. Expected DELETE, PUT or GET"));
                }
            } else {
    ...
    private void handleGetCouponById(HttpExchange exchange, int couponId) throws IOException {
        if (!checkHttpMethod(exchange, "GET")) return;
        try {
    ...
    private void handleUpdateCoupon(HttpExchange exchange, int couponId) throws IOException {
        ...
        try {
            ...
            if (requestBody.getCouponCode() != null) {
                existingCoupon.setCouponCode(requestBody.getCouponCode());
            }
            if (requestBody.getType() != null) {
                existingCoupon.setCouponType(com.aut.shoomal.payment.coupon.CouponType.fromName(requestBody.getType()));
            }
            if (requestBody.getValue() != null) {
                existingCoupon.setValue(requestBody.getValue());
            }
            if (requestBody.getMinPrice() != null) {
                existingCoupon.setMinPrice(requestBody.getMinPrice());
            }
            if (requestBody.getUserCount() != null) {
            ...
            sendRawJsonResponse(exchange, HttpURLConnection.HTTP_OK, convertToCouponResponse(existingCoupon));
        } catch (NotFoundException e) {
            sendResponse(exchange, HttpURLConnection.HTTP_NOT_FOUND, new ApiResponse(false, e.getMessage()));
        } catch (InvalidInputException | IllegalArgumentException | DateTimeParseException e) {
            sendResponse(exchange, HttpURLConnection.HTTP_BAD_REQUEST, new ApiResponse(false, "400 Invalid input: " + e.getMessage()));
        } catch (ConflictException e) {
            sendResponse(exchange, HttpURLConnection.HTTP_CONFLICT, new ApiResponse(false, "409 Conflict: " + e.getMessage()));
        } catch (Exception e) {
